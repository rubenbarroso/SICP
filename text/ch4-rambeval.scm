...
(load "/Users/ruben/Dropbox/SICP/text/ch4-mceval.scm")
...
(define (analyze-ramb exp)
  (analyze-amb
   (cons 'ramb (ramb-choices exp))))

(define (shuffle choices)
  (let ((permutation (shuffle-helper choices)))
    (if (null? (cdr permutation))
        (list (car permutation))
        (cons (car permutation) (shuffle (cdr permutation))))))

(define (shuffle-helper choices)
  (define (iter-shuffle current index rest)
    (if (= index 0)
        (append (list (car rest)) current (cdr rest))
        (iter-shuffle (cons (car rest) current) (- index 1) (cdr rest))))
  (let ((index (random (length choices))))
    (iter-shuffle nil index choices)))
