  ...
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
      ...
      (define (print-instruction-count)
        ...
        (display instruction-count)
        (newline))
      (define (lookup-breakpoint label-name n)
        (let ((insts (lookup-label the-label-sequence label-name)))
          (list-ref insts n)))            
      (define (set-breakpoint! label n)
        (set-instruction-breakpoint! (lookup-breakpoint label n)))        
      (define (cancel-breakpoint! label n)
        (cancel-instruction-breakpoint! (lookup-breakpoint label n))) 
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'cancel-all-breakpoints) cancel-all-breakpoints!) ;TODO
              ((eq? message 'install-labels) 
               (lambda (labels) (set! the-label-sequence labels)))
              ((eq? message 'proceed) 
