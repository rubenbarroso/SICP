...
(define (make-new-machine)
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
        ...
        (let ((insts (get-contents pc)))
          (if (null? insts)
              ...
              (begin
                ...
                (execute)))))
      (define (classify-instruction inst)
        (let* ((type (car (instruction-text inst)))
               (text (instruction-text inst))
               (target (assoc type instructions-by-type)))
          (if (not target)
              (set! instructions-by-type (cons (cons type (list text)) instructions-by-type))
              (if (not (memq text (cdr target)))
                  (set-cdr! target (cons text (cdr target)))))))
      (define (add-entry-point reg)
        (if (not (memq reg entry-points))
      ...
      (define (add-register-source reg-name source)
        (let ((entry (assoc reg-name register-sources)))
          (if entry
              (set-cdr! entry (cons source (cdr entry)))
              (set! register-sources (cons (cons reg-name (list source)) register-sources))))) 
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'instructions-by-type) instructions-by-type)
              ((eq? message 'classify-instruction) classify-instruction)
     ...
     (lambda (inst)
       (set-instruction-execution-proc! 
        ...
        (make-execution-procedure
         ...
         pc flag stack ops))
       ((machine 'classify-instruction) inst))
     insts)))
...
(define (make-assign inst machine labels operations pc)
  (let ((target
    ...
    ((machine 'add-register-source) (assign-reg-name inst) value-exp) ;add register source
    (let ((value-proc
...
(define (make-goto inst machine labels pc)
  (let ((dest (goto-dest inst)))
    (cond ((label-exp? dest)
          ...
          ((register-exp? dest)
           (let* ((register (register-exp-reg dest))
                  (reg (get-register machine register)))
             ((machine 'add-entry-point) register) ; mark as entry point
             (lambda ()
...
(define (make-save inst machine stack pc)
  (let* ((register (stack-inst-reg-name inst))
         (reg (get-register machine register)))
    ((machine 'add-stack-register) register) ; mark as stack-register
    (lambda ()
